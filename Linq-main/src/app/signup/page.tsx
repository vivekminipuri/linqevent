...
export default function SignupPage() {
    ...
    const createProfile = async (user: any) => {
        ...
        const profile: UserProfile = {
            ...
            role: role,
            homeCollegeId: formData.institution, // Using name as ID for MVP
            createdAt: Date.now(),
            // Add specific fields to metadata if needed, strictly typed below
        };

        // We can add extra fields to the doc, though they aren't in UserProfile interface strictly yet
        // For now, we mix them in.
    ...
    const handleManualSignup = async () => {
        if (!formData.email || !formData.password || !formData.institution) {
            ...
            <div className="glass-panel auth-card" style={{ maxWidth: '480px' }}>
                ...
                <div style={{ display: 'flex', background: 'rgba(0,0,0,0.3)', padding: '4px', borderRadius: '12px', marginBottom: '1.5rem' }}>
                    {(['student', 'club_admin', 'college_admin'] as Role[]).map((r) => (
                        ...
                        >
                            {r === 'student' ? 'Student' : r === 'club_admin' ? 'Club Lead' : 'College'}
                        </button>
                    ))}
                </div>
